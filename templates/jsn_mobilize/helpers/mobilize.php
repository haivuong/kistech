...
class JSNMobilizeTemplateHelper
{
	 ...
	 * @return  void
	 */
	public static function renderHtmlBlock($name, $device, $preview, $IDBlock = '', $class = 'row-fluid')
	{
		...
		if (strpos($urlRequest, "jsn-preview-mobilize-" . md5(JURI::root())) == true)
		{
		...
		if (!empty($name))
		{
			...
			if (!empty($htmlBlock))
			{
				...
				echo '<div id="' . $IDBlock . '"' . $class . '><div class="row-fluid">' . $htmlBlock . '</div></div>';
			}
		}
	}
